...
int64 get2Median(const OrderStatisticsMap<pair<int, int>, bool> &t) {
  int n = (int)t.size();
  if (n == 0) {
    return 0;
  } else if (n % 2 == 1) {
    return (int64)2 * t.findByOrder(n / 2).first;
  } else {
    return (int64)t.findByOrder(n / 2 - 1).first + t.findByOrder(n / 2).first;
  }
}
...
int main() {
  ...
  string s;
  int x;
  OrderStatisticsMap<pair<int, int>, bool> t;
  ...
  repeat(n) {
    ...
    if (s[0] == 'r') {
      if (curCount[x] == 0) {
        ...
        t.erase(make_pair(x, curCount[x] - 1));
        curCount[x]--;
        if (t.empty())
          wrong = true;
      }
      ...
      curCount[x]++;
      t.put(make_pair(x, curCount[x] - 1), true);
      if (t.empty())
        wrong = true;
    }
    if (wrong) {
