...
template <class K, class V, class Comparator = less<K>> struct OrderedStatisticsMap {
  ...
  K findByOrder(int p, int x) const {
    int t = 0;
    if (node[p].l != -1)
      t = node[node[p].l].size;
    if (x < t)
      return findByOrder(node[p].l, x);
    x -= t;
    if (x == 0)
      return node[p].key;
    --x;
    return findByOrder(node[p].r, x);
  }
public:
  OrderedStatisticsMap() { root = -1; }
  ...
  K findByOrder(int x) const {
    assert(root != -1 && x >= 0 && x < node[root].size);
    return findByOrder(root, x);
  }
};
...
int main() {
  ...
  sort(a.begin(), a.end());
  OrderedStatisticsMap<pair<int, int>, bool> s;
  vector<int> ret;
  ret.resize(n + 1);
  for (auto &p : a) {
